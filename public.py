...
def get_data(crypto, param, requests_session):
    if requests_session is None:
        requests_session = common.Session()

    url = 'https://vip.bitcoin.co.id/api/'+ crypto + '_idr/'+param
    
    return requests_session.api_request(url)


def getTicker(crypto, session=None):
    """
    ...
    response = get_data(crypto, 'ticker', requests_session=session).json()
    ...
    if not isinstance(response, dict):
        ...
        ticker['vol_'+crypto] = float(response['ticker'].get('vol_'+crypto))
        ticker['server_time'] = str(pd.to_datetime(response['ticker'].get('server_time'), unit='s'))

    return ticker


def getDepth(crypto, session=None):
    """
    ...
    depth = get_data(crypto, 'depth', requests_session=session).json()
...
def getTradeHistory(crypto, session=None):
    """
    ...
    history = get_data(crypto, 'trades', requests_session=session).json()
